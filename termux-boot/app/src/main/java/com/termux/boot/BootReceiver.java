...
public class BootReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        runBootScripts(context);
    }

    /**
     * Scans ~/.termux/boot/ and schedules each script to run, same as a real boot would. Split
     * out of {@link #onReceive} so it can also be triggered on demand -- e.g. from
     * {@code PluginBridge}'s "boot:runNow" action -- without waiting for an actual reboot to
     * test a script.
     *
     * @return a human-readable summary of what was scheduled, for callers that want to report it.
     */
    @SuppressLint("SdCardPath")
    public static String runBootScripts(Context context) {
        final String BOOT_SCRIPT_PATH = "/data/data/com.termux/files/home/.termux/boot";
        ...
        String summary = logMessage.length() > 0
            ? "Executed files at boot: " + logMessage
            : "No files to execute at boot";
        Log.i("termux", summary);
        return summary;
    }
