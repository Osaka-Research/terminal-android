...
public class PackageUtils {
    ...
    /**
     * The termux-clone build merges the api/boot/widget/float plugin apps into this
     * single APK instead of shipping them as separately-installed packages. Code
     * written against the original multi-app layout still looks them up by their
     * old package names (e.g. "com.termux.api") via {@link TermuxConstants} -- since
    ...
    public static Context getContextForPackage(@NonNull final Context context, String packageName, int flags) {
        packageName = resolveMergedPackageName(context, packageName);
        try {
    ...
    public static PackageInfo getPackageInfoForPackage(@NonNull final Context context, @NonNull final String packageName, final int flags) {
        try {
            return context.getPackageManager().getPackageInfo(resolveMergedPackageName(context, packageName), flags);
    ...
    public static ApplicationInfo getApplicationInfoForPackage(@NonNull final Context context, @NonNull final String packageName, final int flags) {
        try {
            return context.getPackageManager().getApplicationInfo(resolveMergedPackageName(context, packageName), flags);
        } catch (final Exception e) {
            return null;
        }
    }
